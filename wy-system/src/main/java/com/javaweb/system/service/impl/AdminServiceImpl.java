...
public class AdminServiceImpl extends ServiceImpl<AdminMapper, Admin> implements IAdminService {
    ...
    public JsonResult getList(BaseQuery query) {
        ...
        if (!adminList.isEmpty()) {
            adminList.forEach(item -> {
                ...
                if (adminListVo.getDataType() != null && adminListVo.getDataType() > 0) {
                    adminListVo.setDataTypeName(AdminConstant.DATA_TYPE_LIST.get(adminListVo.getDataType()));
                }
                // 头像地址
    ...
    public Object getInfo(Serializable id) {
        ...
        // 性别描述
        if (adminInfoVo.getGender() != null && adminInfoVo.getGender() > 0) {
            adminInfoVo.setGenderName(AdminConstant.ADMIN_GENDER_LIST.get(adminInfoVo.getGender()));
        }
        if (adminInfoVo.getDataType() != null && adminInfoVo.getDataType() > 0) {
            adminInfoVo.setDataTypeName(AdminConstant.DATA_TYPE_LIST.get(adminInfoVo.getDataType()));
        }
        return adminInfoVo;
    }
    ...
    public JsonResult edit(Admin entity) {
        ...
        if (entity.getId() != null && entity.getId() > 0) {
            ...
            adminRoleWrapper.eq(AdminRole::getAdminId,entity.getId());
        ...
        for(String roleId : roleIds){
            ...
            adminRole.setCreateUser(ShiroUtils.getAdminId());
            adminRole.setCreateTime(DateUtils.now());
    ...
    public JsonResult deleteByIds(String ids) {
        ...
        boolean result = this.removeByIds(Arrays.asList(item));
        if (!result) {
            return JsonResult.error();
        }
        return JsonResult.success("删除成功");
