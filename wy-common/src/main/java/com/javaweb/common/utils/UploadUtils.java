...
public class UploadUtils {
    ...
    // 文件保存目录路径
    //private String uploadPath = UploadFileConfig.getUploadFolder();
    ...
    // 单个文件最大上传大小(10M)
    private long fileMaxSize = 1024 * 1024 * 10L;
    // 最大文件大小(100M)
    ...
    private String validateFields(HttpServletRequest request, String name) {
        ...
        if (contentType == null || !contentType.startsWith("multipart")) {
            ...
            errorInfo = "上传目录[" + uploadFolder + "]不存在";
        } else if (!uploadDir.canWrite()) {
            errorInfo = "上传目录[" + uploadFolder + "]没有写权限";
        } else if (!extMap.containsKey(dirName)) {
            errorInfo = "目录名不正确";
        } else {
            // 上传路径
            uploadPath = uploadFolder + dirName + "/" + name + "/";
            // 保存目录Url
            ...
            // 创建上传临时目录
            File file = new File(uploadFolder+TEMP_PATH);
            if (!file.exists()) {
                file.mkdirs();
            }
        }
...
//	@SuppressWarnings("unchecked")
    private Map<String, Object> initFields(HttpServletRequest request) {
        ...
        if (isMultipart) {
            ...
            // 设置上传文件的临时目录
            factory.setRepository(new File(uploadFileConfig.getUploadFolder()+TEMP_PATH));
            // 核心操作类:创建一个文件上传解析器。
